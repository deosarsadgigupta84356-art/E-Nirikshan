...
export function checkedOutputPath(target, allowedDirs, label = "screenshot") {
  ...
  // Allow writing into the current repo working directory as a pragmatic
  // fallback when the sandbox `/workspace` isn't available (local runs).
  const cwd = process.cwd();
  const effectiveAllowed = Array.from(new Set([...allowedDirs, cwd]));
  const allowed = effectiveAllowed.some((dir) => abs.startsWith(dir.endsWith(sep) ? dir : dir + sep));
  if (!allowed) {
